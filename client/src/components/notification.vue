<template>
  <v-container pa-0>
    <v-list-tile :key="notification._id" ripple>
      <v-list-tile-content v-if="notification.type === 'followedCollection'">
        <v-list-tile-title>{{
          $t("message.collectionWasFollowed")
        }}</v-list-tile-title>
        <v-list-tile-sub-title class="text--primary">
          <a md12 @click="loadCollection(notification.collection[0])">{{
            notification.collection[0].title
          }}</a>
        </v-list-tile-sub-title>
        <v-list-tile-sub-title
          >{{ $t("message.byUser") }}:
          <a md12 @click="loadUsersTl(notification.user[0]._id)"
            >@{{ notification.user[0].name }}</a
          >, &nbsp; <i>{{ timestamp }}</i>
        </v-list-tile-sub-title>
      </v-list-tile-content>
      ...
      <v-list-tile-content v-if="notification.type === 'unfollowedCollection'">
        <v-list-tile-title>{{
          $t("message.collectionWasUnfollowed")
        }}</v-list-tile-title>
        <v-list-tile-sub-title class="text--primary">
          <a md12 @click="loadCollection(notification.collection[0])">{{
            notification.collection[0].title
      ...
      <v-list-tile-content v-if="notification.type === 'invitationAccepted'">
        <v-list-tile-title>{{
          $t("message.invitationToCollectionAccepted")
        }}</v-list-tile-title>
        <v-list-tile-sub-title class="text--primary">
          <a md12 @click="loadCollection(notification.collection[0])">{{
            notification.collection[0].title
      ...
      <v-list-tile-content v-if="notification.type === 'newPostInCollection'">
        <v-list-tile-title>{{
          $t("message.newPostInThisCollection")
        }}</v-list-tile-title>
        <v-list-tile-sub-title class="text--primary">
          <a md12 @click="loadCollection(notification.collection[0])">{{
            notification.collection[0].title
          ...
          <i
            ><a md12 @click="loadPost(notification.postId)">{{
              notification.text
            }}</a></i
          >
        </v-list-tile-sub-title>
      ...
      <v-list-tile-content v-if="notification.type === 'replyToMyPost'">
        ...
        <v-list-tile-title
          v-if="notification.userCreated === $store.state.user._id"
          >{{ $t("message.aReplyToYourPostPublished") }}
        </v-list-tile-title>
      ...
      <v-list-tile-content v-if="notification.type === 'invitedToFeatureChat'">
        ...
        <v-list-tile-sub-title
          >{{ $t("message.byUser") }}:
          <a md12 @click="loadUsersTl(notification.byUser)"
            >@{{ notification.senderName }}</a
          >, &nbsp; <i>{{ timestamp }}</i>
        </v-list-tile-sub-title>
      </v-list-tile-content>

      <v-list-tile-action v-if="notification.type === 'replyToMyPost'">
        ...
        >
          <v-icon v-if="notification.read === 0" color="orange lighten-1"
            >warning</v-icon
          >
        </v-btn>
        <v-icon v-if="notification.read === 1" color="green darken-2"
          >done</v-icon
        >
      </v-list-tile-action>

      <v-list-tile-action v-if="notification.type === 'newPostInCollection'">
        <v-list-tile-action-text>{{
          notification.action
        }}</v-list-tile-action-text>
        <v-btn
      ...
      <v-list-tile-action
        v-if="
          notification.type === 'unfollowedCollection' ||
      ...
      <v-list-tile-action v-if="notification.type === 'invitedToCollection'">
        ...
        <v-btn
          ...
          small
          color="green"
          @click="invitationAccepted(notification.collectionId)"
        >
          {{ $t("message.accept") }}
        </v-btn>
        <v-btn
          ...
          small
          color="grey"
          @click="invitationDeclined(notification.collectionId)"
        >
          {{ $t("message.decline") }}
        </v-btn>
        <v-icon v-if="notification.read === 1" color="green darken-2"
      ...
      <v-list-tile-action v-if="notification.type === 'invitedToFeatureChat'">
        ...
        <v-btn
          ...
          color="green"
          @click="
            invitationToFeatureChatAccepted(
...
export default {
  ...
  computed: {
    timestamp() {
      ...
      return moment
        .utc(this.notification.timestamp)
        .local()
  ...
  methods: {
    ...
    loadFeatures(newFeature) {
      ...
      if (
        newFeature !== '{"type":"FeatureCollection","features":[]}' &&
        newFeature !== null
      ) {
        let allLayers = [];
        ...
        allLayers.forEach(layer => {
          if (layer.getProperties().name === "customLayer") {
            ...
            layer.getSource().forEachFeature(feature => {
              ...
              if (
                feature.get("mongoID") ===
                AddedFeature[0].getProperties().mongoID
              ) {
                alreadyExists = true;
              }
            });
