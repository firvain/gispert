<template>
  <v-container v-if="$store.state.isUserLoggedIn" id="questionnaires">
    <v-layout row wrap>
      <v-flex v-if="$store.state.questionnaireMode === 'normal'" xs12>
        <h2>{{ $t("message.questionnairesIcreated") }}</h2>
        <v-btn block @click="createNewQuestionnaire()">{{
          $t("message.createQuestionnaire")
        }}</v-btn>
        <v-list three-line>
            ...
            <v-list-tile :key="item._id">
              <v-list-tile-content>
                ...
                <v-list-tile-sub-title>{{
                  item.properties.introduction.items[1].value
                }}</v-list-tile-sub-title>
              </v-list-tile-content>
              <v-list-tile-action>
              ...
              <v-list-tile-action>
                <v-btn
                  icon
                  ripple
                  @click="openViewerForQuestionnaireResults(item)"
                >
      ...
      <v-flex xs12 sm12 md12>
        <questionnaireEditor
          ...
          :qnnaire="questionnaireForEdit"
        >
        </questionnaireEditor>
      </v-flex>
      <v-flex xs12 sm12 md12>
        <questionnaireView
          v-if="$store.state.questionnaireMode === 'viewer'"
      ...
      <v-flex xs12 sm12 md12>
        <questionnaireResultsViewer
          v-if="$store.state.questionnaireMode === 'resultsViewer'"
          :id="questionnaireForView"
        >
        </questionnaireResultsViewer>
      </v-flex>
      <v-flex xs12 sm12 md12>
        <questionnaireOneResultViewer
          v-if="$store.state.questionnaireMode === 'oneResultViewer'"
    ...
    <v-dialog v-model="sendLinkViaEmailDialog" persistent max-width="800px">
      <v-card>
        ...
        <v-btn
          color="blue darken-1"
          text
          @click.native="sendLinkViaEmailDialog = false"
