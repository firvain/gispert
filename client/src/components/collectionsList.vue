<template>
  <v-container id="collectionList" pa-0>
    <v-container
      v-if="
        this.$store.state.isUserLoggedIn &&
          this.$store.state.openedTimeline === null
      "
      id="list"
    >
      <v-layout row wrap>
        <v-layout row wrap>
          <v-flex md10>
            <v-text-field
              ...
              min="4"
              append-icon="search"
              @keyup.enter="searchInCollections"
            ></v-text-field>
          </v-flex>
          <v-flex md2>
        ...
        <v-flex>
          <v-subheader
            v-if="
              $store.state.isUserLoggedIn &&
                $store.state.openedTimeline === null &&
                mode === 'search'
            "
            inset
          >
          ...
          <v-container
            v-if="$store.state.openedTimeline === null && mode === 'search'"
            fluid
          >
            <v-list
            ...
            >
              <collection
                v-if="
                  $store.state.openedTimeline === null ||
                    openedCollection === collection._id
                "
                :collection="collection"
              ></collection>
            </v-list>
            <v-list v-for="thread in postsQueryResult" :key="thread._id">
              <thread
                v-if="
                  $store.state.openedTimeline === null ||
                    openedCollection === collection._id
      ...
      <v-subheader v-if="mode === 'normal'" inset>
        <v-btn
          fab
          dark
          outline
          small
          color="green"
          @click="addPrivateCollectionCard"
        >
          <v-icon dark>add</v-icon>
        </v-btn>
        <v-tooltip bottom>
          <span slot="activator" class="title">{{
            $t("message.privateCollections")
          }}</span>
          <span>{{ $t("message.privateCollectionsHint") }}</span>
        </v-tooltip>
      </v-subheader>
      <v-container v-if="mode === 'normal'" fluid>
        <form v-if="newPrivateCollectionCard">
          ...
          <v-btn @click="newPrivateCollectionCard = false">{{
            $t("message.cancel")
          }}</v-btn>
        </form>
        <v-list
        ...
        <p
          v-if="
            $store.state.privateCollections.length == 0 &&
              $store.state.isUserLoggedIn
          "
        >
          {{ $t("message.noPrivateCollections") }}
          <v-btn
            fab
            dark
            outline
      ...
      <v-subheader v-if="mode === 'normal'" inset>
        <v-btn
          ...
          small
          color="green"
          @click="addPublicCollectionCard"
        >
          <v-icon dark>add</v-icon>
        </v-btn>
        <v-tooltip bottom>
          <span slot="activator" class="title">{{
            $t("message.publicCollections")
          }}</span>
      ...
      <v-container v-if="mode === 'normal'" fluid>
        <form v-if="newPublicCollectionCard">
          ...
          ></v-text-field>
          <v-btn @click="createCollection">{{ $t("message.save") }}</v-btn>
          <v-btn @click="newPublicCollectionCard = false">{{
            $t("message.cancel")
          }}</v-btn>
        </form>
        <v-list
        ...
        <p
          v-if="
            $store.state.publicCollections.length == 0 &&
              $store.state.isUserLoggedIn
          "
        >
          {{ $t("message.noPublicCollections") }}
          <v-btn
            fab
            dark
            outline
    ...
    <collectionView
      v-if="
        $store.state.openedTimeline !== null &&
          $store.state.openedTimeline.type === 'collection'
      "
    ...
    <userTimeline
      v-if="
        $store.state.openedTimeline !== null &&
          $store.state.openedTimeline.type === 'timeline'
      "
    ...
    <v-container>
      <thread
        v-if="
          $store.state.openedTimeline !== null &&
            $store.state.openedTimeline.type === 'thread' &&
            openThread
        "
        :thread="openThread[0]"
      >
