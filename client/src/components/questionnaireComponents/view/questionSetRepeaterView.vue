<template name="questionSetRepeaterView">
  <v-container v-if="question.type === 'repeatable'" fluid pa-0 ma-0>
    ...
    <v-btn block @click="addQuestionSet(question)">{{
      question.buttonText
    }}</v-btn>
  </v-container>
</template>
            ...
            if (i.activateQuestions && i.activateQuestions.length > 0) {
              ...
              i.activateQuestions.forEach(a => {
                ...
                if (idsTable.find(x => x.oldId === a.id)) {
                  const idToReplaceWith = idsTable.find(x => x.oldId === a.id)
                    .newId;
