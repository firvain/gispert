<template>
  <div id="mw" class="mapStyle">
    <div
      v-if="$store.state.questionnaireMode === 'answering'"
      id="overlayLock"
      :class="$store.state.mapState"
    >
      <p>
        Ο χάρτης ενεργοποιείται όταν χρειάζεται να απαντήσετε σε μια ερώτηση που
        σας ζητάει να υποδείξετε στο χάρτη. Αυτές οι ερωτήσεις περιέχουν κουμπιά
        σαν αυτά:
        ...
        <br />Στην περίπτωση που σας ζητείται να σχεδιάσετε γραμμή ή πολύγωνο
        ισχύει ότι με διπλό κλικ σταματάει η σχεδίαση και με Delete διαγράφεται
        το τελευταίο σημείο που βάλατε. <br />Σε περίπτωση λάθους (πχ σημείο
        μπήκε σε λάθος θέση) ξαναπατήστε το ίδιο κουμπί στην αντίστοιχη ερώτηση
        και ξαναβάλτε το στο χάρτη.
      </p>
    </div>
...
export default {
  ...
  data: () => ({
    ...
    endOfMessages: true,
    colorPicker: "",
    strokeWidthRule: [val => val < 10 || "I believe you!"],
    outlineColor: "",
    fillColor: "",
    strokeWidth: "",
  ...
  watch: {
    "$store.state.feature": function handle() {
      ...
      features.forEach(f => {
        ...
        if (
          ...
          f.getProperties().mongoID === this.$store.state.feature.get("mongoID")
        ) {
    ...
    "$store.state.openedTimeline": function changed() {
      console.log(
        "opened timeline changed so clear map features:: ",
  ...
  mounted() {
      ...
      if (e.which === 46) {
        ...
        olMap.getInteractions().forEach(interaction => {
          ...
          if (
            ...
            interaction.getProperties().name === "LineString"
          ) {
    ...
    olMap.on("pointermove", e => {
      ...
      ).innerHTML = `${this.messageDraw}`;
      const zoomLevel = olMap.getView().getZoom();
      if (zoomLevel >= 18) {
        messagesOverlay.setPosition([
          e.coordinate[0] + 10,
          e.coordinate[1] + 20
        ]);
      }
      ...
      if (zoomLevel >= 5 && zoomLevel < 7) {
        messagesOverlay.setPosition([
          e.coordinate[0] + 25000,
          e.coordinate[1] + 40000
        ]);
      }
      if (zoomLevel >= 0 && zoomLevel < 5) {
        messagesOverlay.setPosition([
          e.coordinate[0] + 80000,
          e.coordinate[1] + 180000
        ]);
      }
    });
  },
  methods: {
    ...
    setBasemap(basemap) {
      ...
      allLayers.forEach(layer => {
        ...
        if (
          layer.getProperties().name !== basemap &&
          layer.getProperties().name !== "customLayer"
        ) {
