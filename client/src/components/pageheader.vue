  ...
  <v-toolbar class="orange lighten-2 no-padding" dark>
    ...
    <v-toolbar-items>
      ...
      <v-btn
        v-if="
          $store.state.isUserLoggedIn === false &&
            $store.state.questionnaireMode !== 'answering'
        "
        text
        @click="showRegisterDialogue"
      >
        {{ $t("message.register") }}
      </v-btn>
      <v-btn
      ...
      <v-btn
        ...
        text
        dark
        @click="dialogNotifications = true"
      >
        <v-badge left color="blue">
          <span v-if="bell.number > 0" slot="badge" color="blue">{{
            bell.number
          }}</span>
      ...
      <v-btn
        v-if="$store.state.isUserLoggedIn === true"
        text
        @click="showProfileDialogue"
      >
        {{ $store.state.user.name }}
      </v-btn>
      <!-- <v-btn text @click='showProfileDialogue' v-if="$store.state.isUserLoggedIn === true">
              {{profile_txt}}
            </v-btn> -->
      <v-btn
        v-if="$store.state.isUserLoggedIn === true"
    ...
    <v-flex v-if="$store.state.questionnaireMode !== 'answering'" md1>
      <v-select
        v-model="language"
        class="top"
        :items="languages"
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            text="text"
            @click.native="dialogRegister = false"
          >
            {{ $t("message.cancel") }}
          </v-btn>
        </v-card-actions>
    ...
    <v-snackbar v-model="snackbarRegisterError" :timeout="5000" color="red">{{
      $t("message.alreadyInUseMessage")
    }}</v-snackbar>

    <v-dialog v-model="dialogLogin" max-width="400">
      <v-card>
        <v-card-title class="headline">{{
          $t("message.accountLogin")
        }}</v-card-title>
        <v-container>
          <v-text-field
          ...
          <v-btn
            color="green darken-1"
            text="text"
            @click.native="dialogLogin = false"
        ...
        <v-btn block text @click="auth('google')">
          <i class="fa fa-fw fa-google"></i>
          Google
        </v-btn>
        <!-- <v-btn block text @click="auth('facebook')">
                <i class="fa fa-fw fa-facebook"></i>
                Facebook
              </v-btn> -->
        <!-- <v-btn block text @click="auth('linkedin')">
                <i class="fa fa-fw fa-linkedin"></i>
                Linkedin
              </v-btn>
              <v-btn block text @click="auth('twitter')">
                <i class="fa fa-fw fa-twitter"></i>
                Twitter
              </v-btn> -->
        <!-- <v-btn @click="auth('github')">Github</v-btn> -->
      </v-card>
      <v-snackbar v-model="snackbarLoginError" :timeout="5000" color="red">{{
        wrongLoginInfo
      }}</v-snackbar>
    </v-dialog>
    ...
    <v-dialog v-model="dialogProfile" max-width="290">
      <v-card>
        ...
        <v-card-text>
          <form name="profile-form" autocomplete="off">
            <!-- <v-text-field
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn dark class="cyan" @click="updateProfile">
            {{ $t("message.save") }}
          </v-btn>
          <v-btn
            color="green darken-1"
            text="text"
            @click.native="dialogProfile = false"
          >
            {{ $t("message.cancel") }}
          </v-btn>
        </v-card-actions>
    ...
    <v-snackbar v-model="snackbarLoggedIn" :timeout="5000" color="green">{{
      $t("message.youAreLoggedIn")
    }}</v-snackbar>
    <v-snackbar v-model="snackbarRegistered" :timeout="5000" color="green">{{
      $t("message.youAreRegistered")
    }}</v-snackbar>
    <v-snackbar v-model="snackbarSetLocale" :timeout="5000" color="green"
      >OK</v-snackbar
    >
    ...
    <v-dialog v-model="landingPageOpen" max-width="700px">
      <v-card>
        ...
        <v-card-text>
          ...
          <div class="body-1">
            Are you in planning industry? Maybe you want to ask for citizen
            opinions. Create questions and share them on social media for larger
            audience
          </div>
          <div class="title">Geodata Storage</div>
          <div class="body-1">
            You need a place to store notes with spatial reference?
          </div>
          <div class="title">Talk about locations</div>
          <div class="body-1">
            You need to talk about a place with a friend or colleague? Organize
            your trip, show them a place, guide them to a location with
            interactive chat on the map.
          </div>
          <div class="title">Real-time mapping</div>
          <div class="body-1">
            Create maps collaborating in real time with friends or colleagues.
          </div>
          <v-flex>
            <v-btn
...
export default {
  ...
  data() {
    return {
      ...
      nameRules: [v => !!v || this.$t("message.userNameMissing")],
      passRules: [
      ...
      emailRules: [
        ...
        v =>
          ...
          this.$t("message.emailErrorFormat")
      ],
    ...
    login(credentials) {
      ...
      axios
        ...
        .then(response => {
          ...
          if (
            response.data.error &&
            response.data.error === "Login information was incorrect"
          ) {
          ...
          if (response.data.user) {
            ...
            this.$i18n.locale = response.data.user.locale;
          }
        })
        .then(() => {
      ...
      try {
        ...
        axios
          ...
          .then(() => {
            ...
            console.log(
              "joining collections",
              JSON.stringify(this.privateCollections)
            );
